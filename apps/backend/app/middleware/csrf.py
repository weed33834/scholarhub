...
def _ensure_csrf_cookie(response: Response) -> None:
    """Make sure the response sets a csrf cookie the client can echo back."""
    if "csrf" in response.headers.get("set-cookie", "").lower():
        return
    # The actual cookie value is generated by Starlette via set_cookie()
    # when a previous request's response had ``Set-Cookie``; we just
    # attach a stable opaque value here. ``secrets.token_urlsafe``
    # produces 32 chars which is plenty for a CSRF nonce.
    import secrets
